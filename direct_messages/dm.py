...
class InstaDM(object):
    ...
    def __init__(self, username, password, proxy=False, headless=True, instapy_workspace=None, profileDir=None):
        self.selectors = {
    ...
    def redirect(self, post, users):
        # logging.info(f'Send message to {user}')
        # print(f'Send message to {user}')
        self.driver.get(post)
        self.__random_sleep__(1, 2)
        direct_css_button = self.driver.find_element_by_css_selector(
            "section._9eogI.E3X2T main.SCxLW.o64aR:nth-child(2) div.Kj7h1.yJx9G div.ltEKP article.QBXjJ.M9sTE.L_LMM.JyscU.ePUX4 div.eo2As section.ltpMr.Slqrh span._5e4p > button.wpO6b")

        # direct_button = self.driver.find_elements_by_xpath("//body/div[@id='react-root']/section[1]/main[1]/div[1]/div[1]/article[1]/div[3]/section[1]/span[3]/button[1]")
        direct_css_button.click()
        try:
            for user in users:
                user = user.rstrip()
                ...
                if elements and len(elements) > 0:
                    self.__random_sleep__(2, 3)
                    elements[0].click()
            send_button = self.driver.find_elements_by_xpath("//body/div[5]/div[1]/div[1]/div[2]/div[4]/button[1]")
            send_button[0].click()
            self.__random_sleep__(1, 2)
